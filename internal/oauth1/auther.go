...
func newAuther(consumerKey, consumerSecret string) *auther {
	return &auther{
...
func (a *auther) setRequestAuthHeader(req *http.Request, token, tokenSecret string) error {
	fmt.Println(111)
	oauthParams := a.commonOAuthParams()
	oauthParams[oauthTokenParam] = token
	params, err := collectParameters(req, oauthParams)
	if err != nil {
		return err
	}
...
func (a *auther) commonOAuthParams() map[string]string {
	...
	params := map[string]string{
		oauthConsumerKeyParam:     a.consumerKey,
...
func (a *auther) signer() Signer {
	...
	// }
	return &HMACSigner{ConsumerSecret: a.consumerSecret}
}
